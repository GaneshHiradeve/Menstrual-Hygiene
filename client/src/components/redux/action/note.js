...
export const Createtask=(title,description,folder,color)=>async(dispatch)=>{
   try{
       dispatch({type:"usernoteRequest"});
       
       const {data} =await axios.post(`${server}/note`,{
        title,description,folder,color
       },{
         headers:{
       ...
       dispatch({type:'usernoteSuccess',payload:data})
   }catch(err){
     dispatch({type:'usernoteFail',payload:err.response.data.message})
...
export const updateNote=(updatetitle,updatedescription,note_id)=>async(dispatch)=>{
     
  try{
           dispatch({type:"updatenoteRequest"});

           const {data}=await axios.put(`${server}/${note_id}`,{
...
export const ChangeColor=(color,note_id)=>async(dispatch)=>{
  ...
  try{
           ...
           const {data}=await axios.put(`${server}/colorchange/${note_id}`,{
                    color
           },
